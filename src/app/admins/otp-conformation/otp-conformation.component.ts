          ...
          if (arr.length > 0) {
            if (arr[0].email_verified === 0) {
              this.alertObj.infoAlert(
                ...
                (dt) => {
                  this.router.navigate(['login'],{relativeTo:this.activatedrouteObj});
                }
                );
              ...
              if (this.masterObj.userType === 0) {
                ...
                this.router.navigate(['../jobs/dashboard'],{relativeTo:this.activatedrouteObj});
              } else if (this.masterObj.userType === 2) {
                this.router.navigate(['../hostel/dashboard'],{relativeTo:this.activatedrouteObj});
              } else if (this.masterObj.userType === 3) {
                this.router.navigate(['../institute/institute-dashboard'],{relativeTo:this.activatedrouteObj});
              }
            }
          }
        },
    ...
    this.masterObj.getverifyPhone(this.masterObj.unquikey, this.txtotp.value).subscribe(
      (data: Data) => {
        ...
        if (res.status === 1) {
          ...
          this.alertObj.errorAlert( res.message);
        }
      },
      (error) => {
        if (error.status === 404) {
