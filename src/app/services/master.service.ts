...
export class MasterService {
  ...
  getStates(id) {
    return this.http.get<Data>(this.url + '/common/states/' + id).pipe(
      retry(0),
  ...
  getCities(id) {
    // id , city_name
    return this.http.get<Data>(this.url + '/common/cities/' + id).pipe(
      retry(0),
  ...
  profileUpdate(options) {
    return this.http.put<Data>(this.url + '/users/update', options).pipe(
      retry(0),
